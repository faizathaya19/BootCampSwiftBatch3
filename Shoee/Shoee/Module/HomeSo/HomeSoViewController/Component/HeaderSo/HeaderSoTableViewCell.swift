...
class HeaderSoTableViewCell: BaseTableCell {
    ...
    func configureHeader(nameim: String, name: String?, username: String?, imageURLString: String?, skeletonView: Bool) {
        nameLabel.text = name
        usernameLabel.text = username
        isSkeletonable = skeletonView

        let encodedName = nameim.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
